...
export default function ComplianceMetricsPage() {
  ...
  const metrics = useMemo(() => {
    return employees.map(emp => {
      const empTasks = tasks.filter(t => t.assignedTo === emp.name || t.assignedTo === emp.id);
      const completedTasks = empTasks.filter(t => t.status === "Completed").length;
      const taskCompRate = empTasks.length > 0 ? Math.round((completedTasks / empTasks.length) * 100) : 100;

      const empExpenses = expenses.filter(e => e.userName === emp.name);
      ...
      // Mock attendance compliance
      const lateCount = Math.floor(Math.random() * 3);
      const attendanceCompRate = Math.max(0, 100 - lateCount * 15);
