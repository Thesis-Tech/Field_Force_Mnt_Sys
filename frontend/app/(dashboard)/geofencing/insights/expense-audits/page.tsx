...
export default function ExpenseAuditsPage() {
  ...
  const pendingClaims = expenses.filter(e => e.status === "Pending Approval by Manager");
  const rejectedClaims = expenses.filter(e => e.status === "Rejected");

  const totalExpenseBurn = approvedClaims.reduce((acc, exp) => acc + exp.amount, 0);
  const pendingSum = pendingClaims.reduce((acc, exp) => acc + exp.amount, 0);
  ...
  const topCategory = [...categorySums].sort((a, b) => b.amount - a.amount)[0];
